...
void RosAdapter::Init() {
  ...
  {
    // arena_info_sub_ = nh_->create_subscription<vehicle_msgs::msg::ArenaInfo>(
    //     "arena_info", 2, &RosAdapter::ArenaInfoCallback, this,
    //     rclcpp::TransportHints().tcpNoDelay());

    arena_info_static_sub_ =
        nh_->create_subscription<vehicle_msgs::msg::ArenaInfoStatic>(
            "arena_info_static", rclcpp::QoS(2).reliable(),
            std::bind(&RosAdapter::ArenaInfoStaticCallback, this,
                      std::placeholders::_1));
    arena_info_dynamic_sub_ =
        nh_->create_subscription<vehicle_msgs::msg::ArenaInfoDynamic>(
            "arena_info_dynamic", rclcpp::QoS(2).reliable(),
            std::bind(&RosAdapter::ArenaInfoDynamicCallback, this,
                      std::placeholders::_1));
  }
}
...
void RosAdapter::ArenaInfoCallback(
    const vehicle_msgs::msg::ArenaInfo::SharedPtr msg) {
  rclcpp::Time time_stamp;
  vehicle_msgs::Decoder::GetSimulatorDataFromRosArenaInfo(
      *msg, &time_stamp, &lane_net_, &vehicle_set_, &obstacle_set_);
  p_data_renderer_->Render(time_stamp.seconds(), lane_net_, vehicle_set_,
                           obstacle_set_);
