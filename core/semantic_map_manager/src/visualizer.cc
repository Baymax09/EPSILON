...
Visualizer::Visualizer(rclcpp::Node::SharedPtr nh, int node_id)
  ...
  ego_vehicle_pub_ =
      nh_->create_publisher<visualization_msgs::msg::MarkerArray>(ego_vehicle_vis_topic, 1);
  obstacle_map_pub_ =
      nh_->create_publisher<nav_msgs::msg::OccupancyGrid>(obstacle_map_vis_topic, 1);
  surrounding_lane_net_pub_ = nh_->create_publisher<visualization_msgs::msg::MarkerArray>(
      surrounding_lane_net_vis_topic, 1);
  local_lanes_pub_ =
      nh_->create_publisher<visualization_msgs::msg::MarkerArray>(local_lanes_vis_topic, 1);
  behavior_vis_pub_ = nh_->create_publisher<visualization_msgs::msg::MarkerArray>(
      ego_vehicle_behavior_topic, 1);
  pred_traj_openloop_vis_pub_ = nh_->create_publisher<visualization_msgs::msg::MarkerArray>(
      pred_traj_openloop_topic, 1);
  pred_intention_vis_pub_ =
      nh_->create_publisher<visualization_msgs::msg::MarkerArray>(pred_intention_topic, 1);
  surrounding_vehicle_vis_pub_ = nh_->create_publisher<visualization_msgs::msg::MarkerArray>(
      surrounding_vehicle_topic, 1);
  speed_limit_vis_pub_ =
      nh_->create_publisher<visualization_msgs::msg::MarkerArray>(speed_limit_topic, 1);
}
...
void Visualizer::VisualizeObstacleMap(
  ...
  common::VisualizationUtil::GetRosOccupancyGridUsingGripMap2D(
      obstacle_map, rclcpp::Clock().now(), &occ_map);
...
void Visualizer::SendTfWithStamp(const rclcpp::Time& stamp,
  ...
  tf_msg.child_frame_id = ego_tf_name_;
  // translation
  ...
  // publish TF
  ego_to_map_tf_->sendTransform(tf_msg);
}
...
void Visualizer::VisualizeSurroundingLaneNet(
  ...
  // visualization_msgs::msg::MarkerArray surrounding_lane_net_marker;
  // int id_cnt = 0;
